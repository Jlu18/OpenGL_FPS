...
void Renderer::Init()
{
	...
	r_data.shader->SetVec4f("u_LightColor", glm::vec4(1.f, 1.f, 1.f, 1.0f));
...
void Renderer::PreRender(const Camera& cam) {
	r_data.shader->Bind();
	r_data.shader->SetMat4f("u_ViewProjMatrix", cam.GetViewProjMatrix());
...
void Renderer::SetLight(const Light& light, const Transform& transform) {
	r_data.shader->SetVec3f("u_LightPos", transform.position);
}

//draw box only now
void Renderer::Render(const Transform& transform, const Renderable& renderable)
{
	r_data.shader->SetVec4f("u_Color", renderable.color);
	r_data.shader->SetMat4f("u_Matrix", transform.GetTransformMatrix());
